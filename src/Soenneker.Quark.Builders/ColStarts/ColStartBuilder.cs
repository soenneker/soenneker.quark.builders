...
{
    internal ColStartBuilder(string value, BreakpointType? breakpoint = null) : base("col-start-", value, breakpoint)
    {
    }

    public ColStartBuilder Auto => ChainValue("auto");
    public ColStartBuilder At(int value) => ChainValue(value.ToString());
    public ColStartBuilder Token(string value) => ChainValue(value);
