...
{
    internal ColEndBuilder(string value, BreakpointType? breakpoint = null) : base("col-end-", value, breakpoint)
    {
    }

    public ColEndBuilder Auto => ChainValue("auto");
    public ColEndBuilder At(int value) => ChainValue(value.ToString());
    public ColEndBuilder Token(string value) => ChainValue(value);
